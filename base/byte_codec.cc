...
DataReader::DataReader(const char* data,const size_t len,Endianness endianness)
...
bool DataReader::ReadVarInt62(uint64_t* result){
    DCHECK_EQ(endianness(),NETWORK_BYTE_ORDER);
...
DataWriter::DataWriter(char* buffer,size_t size):DataWriter(buffer,size,NETWORK_BYTE_ORDER){}
DataWriter::DataWriter(char* buffer,  size_t size,Endianness endianness)
